...
from detectron2.structures.masks import MyMasks
...
def transform_instance_annotations(
    annotation, transforms, image_size, manual_reweight, weights_dir, *, keypoint_hflip_indices=None
):
    """
    ...
    #我自己加的
    if True: #manual_reweight 这里true是为了ablation中的only branch:
        annotation["abn"] = np.load(weights_dir + "/{}.npy".format(annotation["id"]))
        annotation["abn"] = transforms.apply_image(annotation["abn"])
...
def annotations_to_instances(annos, image_size, manual_reweight, weights_dir, mask_format="polygon"):
    """
    ...
    if len(annos) and "id" in annos[0]:
        id_list = [int(obj["id"]) for obj in annos]
        ids = torch.tensor(id_list, dtype=torch.int64)
        target.gt_ids = ids
    if len(annos) and "abn" in annos[0]:# and manual_reweight, for branch only.:
        abn_list = [obj["abn"] for obj in annos]
        abn_list = [torch.from_numpy(np.ascontiguousarray(x)) for x in abn_list]
        abns = torch.stack(abn_list)
        abns = MyMasks(abns)
        target.gt_abns = abns
