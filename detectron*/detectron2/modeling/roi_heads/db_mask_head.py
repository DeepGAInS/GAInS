...
__all__ = [
    "BaseMaskRCNNHead",
    "DoubleBranchMaskHead",
    "build_double_branch_mask_head",
...
def mask_rcnn_inference(pred_mask_logits: torch.Tensor, pred_instances: List[Instances]):
    ...
    # for i, map in enumerate(mask_probs_pred):
    #     mask = map.detach().cpu().numpy()
    #     for j in range(mask.shape[0]):
    #         plt.imshow(mask[j][0], cmap='grey')
    #         plt.savefig(f'mask_{i}{j}.png')
...
class BaseMaskRCNNHead(nn.Module):
    ...
    @configurable
    def __init__(self, *, loss_weight: float = 1.0, weights_dir: str, manual_reweight: bool, vis_period: int = 0):
        """
        ...
        self.manual_reweight = manual_reweight

    @classmethod
    def from_config(cls, cfg, input_shape):
        return {"vis_period": cfg.VIS_PERIOD,
                "weights_dir": cfg.MODEL.ROI_HEADS.WEIGHTS_DIR,
                "manual_reweight": cfg.MODEL.ROI_HEADS.MANUAL_REWEIGHT}
...
class DoubleBranchMaskHead(BaseMaskRCNNHead):
    """
    ...
    def __init__(self, cfg, input_shape: ShapeSpec):
        ...
        self.norm         = cfg.MODEL.ROI_DOUBLE_BRANCH_MASK_HEAD.NORM
        num_conv          = cfg.MODEL.ROI_DOUBLE_BRANCH_MASK_HEAD.NUM_CONV
        input_channels    = input_shape.channels
        cls_agnostic_mask = cfg.MODEL.ROI_DOUBLE_BRANCH_MASK_HEAD.CLS_AGNOSTIC_MASK
    ...
    def forward(self, x, instances):
        ...
        if self.training:
            return {"loss_masks": mask_rcnn_loss(x, instances, self.weights_dir, self.manual_reweight, self.vis_period)}, x, inner_feature
        else:
            mask_rcnn_inference(x, instances)
            return instances, x, inner_feature

def build_double_branch_mask_head(cfg, input_shape):
    """
    Build a mask head defined by `cfg.MODEL.ROI_DOUBLE_BRANCH_MASK_HEAD.NAME`.
    """
    name = cfg.MODEL.ROI_DOUBLE_BRANCH_MASK_HEAD.NAME
