...
class MyMasks(BitMasks):
    ...
    @torch.jit.unused
    def to(self, *args: Any, **kwargs: Any) -> "MyMasks":
        return MyMasks(self.tensor.to(*args, **kwargs))
    
    @torch.jit.unused
    def __getitem__(self, item: Union[int, slice, torch.BoolTensor]) -> "MyMasks":
        """
        Returns:
            BitMasks: Create a new :class:`MyMasks` by indexing.
        ...
        """
        if isinstance(item, int):
            return MyMasks(self.tensor[item].unsqueeze(0))
        m = self.tensor[item]
        assert m.dim() == 3, "Indexing on MyMasks with {} returns a tensor with shape {}!".format(
            item, m.shape
        )
        return MyMasks(m)
    ...
    def crop_and_resize(self, boxes: torch.Tensor, mask_size: int) -> torch.Tensor:
        ...
        Returns:
            Tensor:
                A float tensor of shape (N, mask_size, mask_size), where
                N is the number of predicted boxes for this image.
        """
        assert len(boxes) == len(self), "{} != {}".format(len(boxes), len(self))
        device = self.tensor.device
